...
@EventBusSubscriber(modid = "magicindustries")
public class ModEvents {
   ...
   public static void addCustomTrades(VillagerTradesEvent event) {
      if (event.getType() == VillagerProfession.FARMER) {
         ...
         trades.get(4).add((pLevel, pTrader, pRandom) -> new MerchantOffer(new ItemCost(Items.EMERALD, 32), new ItemStack((ItemLike)ModBlocks.MANA_FLOWER.get(), 1), 1, 8, 0.02F));
      }
   }

   @SubscribeEvent
   public static void addCustomWanderingTrades(WandererTradesEvent event) {
      List<ItemListing> genericTrades = event.getPools().get(0).getEntries();
      genericTrades.add((pLevel, pTrader, pRandom) -> new MerchantOffer(new ItemCost(Items.EMERALD, 16), new ItemStack((ItemLike)ModBlocks.MANA_FLOWER.get(), 1), 1, 8, 0.2F));
   }
   ...
   public static void onAttachCapabilitiesPlayer(AttachCapabilitiesEvent.Entities event) {
      if (event.getObject() instanceof Player) {
         if (!event.getObject().getCapability(PlayerManaProvider.PLAYER_MANA).isPresent()) {
            event.addCapability(Identifier.fromNamespaceAndPath(MagicIndustries.MOD_ID, "properties"), new PlayerManaProvider());
         }
      }
   }
}